...
test('brain blocks high spend without explicit approval', () => {
  const brain = createDonBrain({
    budget: { travel: { limit: 1000, spent: 0 } },
  });
...
test('brain allows approved high spend and marks risk', () => {
  const brain = createDonBrain({
    budget: { groceries: { limit: 100, spent: 0 } },
  });
...
test('brain queues managed tasks and updates snapshot', () => {
  const brain = createDonBrain();
  ...
  assert.match(response.summary, /Plan optimized/);
});
